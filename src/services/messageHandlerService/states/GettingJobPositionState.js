...
export default class GetJobPositionState extends BaseState {
  constructor({ whatsappService, sessionService, messageValidator, config, nextState }) {
  ...
  async handle({ from, message, messageId }) {
    ...
    this.sessionTracker.updateData(from, { jobPosition });
    ...
    const audioId = await this.whatsappClient.uploadMedia(await this.getAudioStream("./src/assets/audios/audio1.mp3"), "audio/mp3");
    await this.whatsappClient.sendMediaMessage(from, "audio", audioId, "");
    
    // Update session step
    this.sessionTracker.updateStep(from, this.nextState);
  }
